...
const StarRating = ({rating, count}) => {
  return (
    <div className="flex items-center">
      {[1, 2, 3, 4, 5].map((ele) => {
        if (ele <= rating) return <Star key={ele} />;
        else if (ele - rating < 1) return <Star key={ele} half={true} />;
        else return <Star key={ele} empty={true} />;
      })}
      {Boolean(count) && <span className="ml-2">({count})</span>}
    </div>
  );
};
...
const Star = ({ half = false, empty = false }) => {
  if (half)
    return (
      <svg
