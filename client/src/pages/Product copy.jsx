...
const ProductInfo = ({
  ...
  const [cart, setCart] = useState(cartItem_id);
  ...
  useEffect(() => {
    updateAvailableAttributes(selectedAttributes);
  }, [selectedAttributes]);

  const updateAvailableAttributes = (selectedAttrs) => {
    let newAvailableAttrs = {};
    let matchedVariants = variants.filter((variant) =>
    ...
    matchedVariants.forEach((variant) => {
      variant.attributes.forEach(({ name, value }) => {
        if (!newAvailableAttrs[name]) {
          newAvailableAttrs[name] = new Set();
        }
        newAvailableAttrs[name].add(value);
      });
    });

    // Automatically select related attributes if the current selection is not valid
    let newSelectedAttrs = { ...selectedAttrs };
    Object.keys(newAvailableAttrs).forEach((key) => {
      if (!newAvailableAttrs[key].has(newSelectedAttrs[key])) {
        newSelectedAttrs[key] = [...newAvailableAttrs[key]][0];
      }
    });

    // setSelectedAttributes(newSelectedAttrs);
    setAvailableAttributes(newAvailableAttrs);
  };
  ...
  const getQuantity = () => {
    ...
    return selectedVariant ? selectedVariant.quantity : quantity;
  };

  const isAvailable = (name, value) => {
    return availableAttributes[name]?.has(value);
  };
  ...
  const handleRemoveFromCart = async () => {
    ...
    const { status, message, data } = await removeFromCart_api(token, cart);
  ...
  return (
    <div className="space-y-2 mt-2 w-full sm:w-[30%]">
      ...
      <p className="text-xl">${getPrice()}</p>
      <p className="text-neutral-500 text-sm py-4">{description}</p>
      <Seprator />
      ...
      {/* {attributes.map(({ name, values }, index) => (
        <div key={index} className="flex items-center space-x-2">
          ...
          {values.map((val, idx) => {
            ...
            const available = isAvailable(name, val);
            ...
            return name.toLowerCase() === "color" ? (
              ...
              >
                <div
                  className={`h-full w-full rounded-full bg-${val}-400`}
                ></div>
              </button>
            ) : (
              <button
      ...
      <div className="flex space-x-4 py-2">
        <CountBox {...{ quantity: getQuantity(), countRef }} />
        <Button
          className="sm:text-sm"
          onClick={() => {
            if (cart == undefined) handleAddtoCart();
            else handleRemoveFromCart();
          }}
        >
          {cart == undefined ? "Add To Cart" : "Remove From Cart"}
        </Button>
        <Button variant="outline">
          {wishlist == undefined ? (
            <img
...
const CountBox = ({ quantity, countRef }) => {
